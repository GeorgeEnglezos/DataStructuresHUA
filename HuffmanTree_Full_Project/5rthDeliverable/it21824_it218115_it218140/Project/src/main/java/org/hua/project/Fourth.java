    ...
    private static StringBuilder encodeInputFile(String inputFile) throws IOException {
        ...
        while ((currentLine = inputFileStream.readLine()) != null) {
            ...
            for (int i = 0; i < currentLine.length(); i++) {
                String code = getCode(codeList, currentLine.charAt(i)); //get the code of the character
                encodedText.append(code);
            }
            arr[currentLine.length()]='\n';
            String code = getCode(codeList, arr[currentLine.length()]); //get the code of the character
            encodedText.append(code);
        }
        for (code c:codeList) {
            if (c.getC()==-1){
